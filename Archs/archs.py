...
class AttentionFusion(nn.Module):
    def __init__(self, dim):
        ...
        self.layers= Attention(dim)

    def forward(self, x):
        bs, dim, wid, hei = x.shape[0], x.shape[1] , x.shape[2], x.shape[3]
        
        gp = 1
        x = x.reshape(bs, gp, dim, wid, hei)
        x = x.permute(0, 1, 3, 4, 2).reshape(bs, gp * wid * hei, dim)
        x = self.layers(x)
        x = x.reshape(bs, gp, wid, hei, dim).permute(0, 1, 4, 2, 3).reshape(bs, dim, wid, hei)

        return x

class ConvFusionBlock(nn.Module):
    def __init__(self, in_ch, out_ch, ke_si, padding):
        super(ConvFusionBlock, self).__init__()

        self.conv1 = nn.Conv2d(in_ch, out_ch, 3, padding=padding)
       
        self.conv2 = nn.Conv2d(in_ch, out_ch, 3, padding=padding)
        self.fusion = AttentionFusion(2*out_ch)
...
class Fusion_DoubleConv(nn.Module):
    def __init__(self, in_ch, out_ch):
        super(Fusion_DoubleConv, self).__init__()
        self.conv = nn.Sequential(
            ConvFusionBlock(in_ch, out_ch, 3, padding=1),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True),
            ConvFusionBlock(out_ch, out_ch, 3, padding=1),
            nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True)
        )
...
class Fusion_D_DoubleConv(nn.Module):
    def __init__(self, in_ch, out_ch):
        super(Fusion_D_DoubleConv, self).__init__()
        self.conv = nn.Sequential(
            ConvFusionBlock(in_ch, in_ch, 3, padding=1),
            nn.BatchNorm2d(in_ch),
            nn.ReLU(inplace=True),
            ConvFusionBlock(in_ch, out_ch, 3, padding=1),
            nn.BatchNorm2d(out_ch),
...
class Rolling_Unet_S(nn.Module):
    def __init__(self, num_classes, input_channels=3, deep_supervision=False, img_size=224,
        ...
        self.conv1 = DoubleConv(input_channels, embed_dims[0],kernel_size = 3, padding = 1)
        self.pool1 = nn.MaxPool2d(2)
        self.conv2 = DoubleConv(embed_dims[0], embed_dims[1])
        self.pool2 = nn.MaxPool2d(2)
        self.conv3 = Fusion_DoubleConv(embed_dims[1], embed_dims[2])
        self.pool3 = nn.MaxPool2d(2)
        ...
        self.decoder3 = Fusion_D_DoubleConv(embed_dims[2], embed_dims[1])
        self.decoder2 = D_DoubleConv(embed_dims[1], embed_dims[0])
        self.decoder1 = D_DoubleConv(embed_dims[0], 8)
